# vsg_core/models/context.py
"""
Pipeline execution context model.

This module contains the Context dataclass that holds all state
for a pipeline execution run. Previously in vsg_core/orchestrator/steps/context.py.
"""
...
class Context:
    ...
    Holds all state needed during a pipeline run, including settings,
    extracted items, delays, and various correction flags.
    """
    ...
    # Format: {'Source 1': {'correlation_ref_track': 0}, ...}
    ...
    # Track which sources had stepping detected (for final report)
    stepping_sources: list[str] = field(default_factory=list)

    # Track sources where stepping was detected but correction is disabled
    stepping_detected_disabled: list[str] = field(default_factory=list)

    # Track sources where stepping was detected but skipped due to source separation
    ...
    # Format: [{'source': str, 'issue_type': str, 'severity': str, ...}]
    ...
    # Format: [{'source': str, 'variance_detected': bool, ...}]
