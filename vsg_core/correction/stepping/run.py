...
# ---------------------------------------------------------------------------
# generate_edl_from_correlation (subtitle-only, kept from old stepping.py)
# ---------------------------------------------------------------------------
...
def generate_edl_from_correlation(
        ...
        if correction_mode == "filtered":
            ...
            if invalid_clusters:
                ...
                filtered = [
                    c
                    for c in accepted
                    if not any(s <= c["start"] <= e for s, e in invalid_time_ranges)
                ]
                runner._log_message(
                    f"[EDL Generation] Filtered {len(accepted) - len(filtered)} "
                    f"chunks from invalid clusters"
                )
                accepted = filtered
                if not accepted:
                    runner._log_message(
    ...
    for chunk in accepted[1:]:
        if abs(chunk["delay"] - current_delay_ms) > tolerance_ms:
            boundary_time_s = chunk["start"]
