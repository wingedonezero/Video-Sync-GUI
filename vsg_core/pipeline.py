...
class JobPipeline:
    ...
    def run_job(
        ...
        # FIX: Prevent the logger's messages from bubbling up to the root logger/terminal
        ...
        try:
            ...
            ctx = orch.run(
                ...
                ref_file=ref_file, sec_file=sec_file, ter_file=ter_file, and_merge=and_merge,
            ...
            if not and_merge:
                ...
                self.progress(1.0)
                return {
                    'status': 'Analyzed', 'delay_sec': ctx.delay_sec_val,
                    'delay_ter': ctx.delay_ter_val, 'name': Path(ref_file).name
                }
            ...
            self.progress(1.0)
            return {
                'status': 'Merged', 'output': str(out_file), 'delay_sec': ctx.delay_sec_val,
                'delay_ter': ctx.delay_ter_val, 'name': Path(ref_file).name
            }
