...
class ExtractStep:
    def run(self, ctx: Context, runner: CommandRunner) -> Context:
        ...
        for item in ctx.manual_layout:
            ...
            if track_type == "video" and source != "Source 1":
                runner._log_message(
                    f"[WARNING] Skipping video track from {source} (ID {item.get('id')}). Video is only allowed from Source 1."
                )
                continue
        ...
        for source_key, source_path in ctx.sources.items():
            ...
            if info:
                delays_for_source = {}
                runner._log_message(
                    f"[Container Delays] Reading delays from {source_key}:"
                )
                    ...
                    for stream in ffprobe_data.get("streams", []):
                        if stream.get("codec_type") == "video":
                            ...
                            if dar:
                                ...
                                runner._log_message(
                                    f"[{source_key}] Video track {stream_index} aspect ratio: {dar}"
                                )

                    source_aspect_ratios[source_key] = aspect_ratios_for_source
                except json.JSONDecodeError:
                    runner._log_message(
                        f"[WARNING] Could not parse ffprobe output for {source_key}"
                    )
        ...
        for source_key, source_path in ctx.sources.items():
            track_ids_to_extract = [
                t["id"] for t in ctx.manual_layout if t.get("source") == source_key
            ]
            if track_ids_to_extract:
                runner._log_message(
                    f"Preparing to extract {len(track_ids_to_extract)} track(s) from {source_key}..."
                )
        ...
        extracted_map: dict[str, dict[str, Any]] = {
            f"{t['source']}_{t['id']}": t for t in all_extracted_tracks
        }
        ...
        for sel in ctx.manual_layout:
            ...
            if source == "External":
                ...
                if not trk:
                    runner._log_message(
                        f"[WARNING] Could not find extracted file for {key}. Skipping."
                    )
                    continue
                ...
                container_delay = ctx.container_delays.get(source, {}).get(
                    int(trk["id"]), 0
                )
            ...
            plan_item.font_replacements = sel.get(
                "font_replacements"
            ...
            plan_item.custom_lang = sel.get(
                "custom_lang", ""
            )  # Preserve custom language
        ...
        failed_generated_tracks = self._process_generated_tracks(
            items, runner, ctx.temp_dir
        )

        # Remove any generated tracks that failed processing (have no extracted_path)
        if failed_generated_tracks:
            runner._log_message(
                f"[WARNING] Removing {len(failed_generated_tracks)} generated track(s) that failed processing"
            )
    ...
    def _process_generated_tracks(
        self, items: list[PlanItem], runner: CommandRunner, temp_dir: Path
    ) -> list[PlanItem]:
        """
        ...
        for item in items:
            ...
            runner._log_message(
                f"[Generated Track] Preparing track from {item.track.source} Track {item.source_track_id}..."
            )
            ...
            for potential_source in items:
                if (
                    potential_source.track.source == item.track.source
                    and potential_source.track.id == item.source_track_id
                    and not potential_source.is_generated
                ):
            ...
            if not source_item:
                ...
                if item.container_delay_ms != 0 or item.sync_to:
                    runner._log_message(
                        f"  Inherited - Container delay: {item.container_delay_ms}ms, Sync to: {item.sync_to}"
                    )
            ...
            if not source_path or not source_path.exists():
                runner._log_message(
                    f"[ERROR] Source file not found for generated track: {source_path}"
                )
                runner._log_message(
                    f"[ERROR] Removing generated track '{item.custom_name or item.track.props.name}' from pipeline"
                )
            ...
            try:
                ...
                runner._log_message(
                    "  Style filtering will be applied in subtitles step"
                )
