...
def merge_chapter_languages(original_xml: str, normalized_xml: str, runner: CommandRunner) -> str:
    ...
    try:
        ...
        for atom in normalized_root.xpath('//ChapterAtom'):
            ...
            start_time = start_elem.text if start_elem is not None and start_elem.text else None
        ...
        runner._log_message(f'[WARNING] Failed to update chapter timestamps: {e}. Using original chapters as-is.')
        return original_xml


def inject_chapters(mkv_path: Path, chapters_xml: str, runner: CommandRunner, tool_paths: dict):
