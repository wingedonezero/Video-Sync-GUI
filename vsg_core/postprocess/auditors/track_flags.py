...
class TrackFlagsAuditor(BaseAuditor):
    ...
    def run(
        ...
        if default_videos > 1:
            self._report(
                f"Multiple default video tracks found ({default_videos}). "
                "Players may behave unexpectedly."
            )

        if default_audios == 0 and audio_tracks:
            self._report(
                "No default audio track set. Some players may not play "
                "audio automatically."
            )
        elif default_audios > 1:
            self._report(f"Multiple default audio tracks found ({default_audios}).")

        if default_subs > 1:
            self._report(f"Multiple default subtitle tracks found ({default_subs}).")
        ...
        if forced_subs > 1:
            self._report(
                f"Multiple forced subtitle tracks found ({forced_subs}). "
                "Only one should be forced."
            )
