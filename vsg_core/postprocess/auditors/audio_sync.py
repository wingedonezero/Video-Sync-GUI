...
class AudioSyncAuditor(BaseAuditor):
    ...
    def run(
        ...
        if not self.ctx.delays:
            self.log(
                "✅ No delays were calculated (single source or analysis skipped)."
            )
            return 0
        ...
        if sync_mode == "allow_negative":
            self.log(
                "[SYNC MODE] Negative delays are allowed in this mode - validation adjusted accordingly."
            )
    ...
    def _verify_codec_delay(
        ...
        if sync_mode == "allow_negative" and expected_delay_ms < 0:
            ...
            if frame_size_ms is not None:
                if frame_size_ms == 0:
                    ...
                    self.log(
                        "     No frame boundary constraints - delay should be exact"
                    )
                    tolerance_ms = 1.0
                else:
                ...
                tolerance_ms = 100.0
                self.log(
                    f"  ⓘ '{name}' ({source}) has negative delay with unknown frame size (codec: {codec_id})"
                )
        ...
        if diff_ms > tolerance_ms:
            ...
            if (
                ...
                and expected_delay_ms < 0
                and actual_delay_ms >= 0
            ):
                self.log(
                    "          NOTE: Expected negative delay in allow_negative mode, but got positive."
                )
                self.log(
    ...
    def _verify_first_packet_timestamp(
        ...
        try:
                ...
                if frame_size_ms is not None:
                    if frame_size_ms == 0:
                        ...
                        if diff_ms <= tolerance_ms:
                            ...
                            self.log(
                                f"          Difference:              {diff_ms:.1f}ms"
                            )
                            self.log(
                                f"          Tolerance used:          ±{tolerance_ms:.0f}ms"
                            )
                            self.log(f"          Sync mode:               {sync_mode}")
                            self.log(
            ...
            if diff_ms > tolerance_ms:
                ...
                self.log(
                    f"          Expected first packet at: {expected_delay_ms:+.1f}ms"
                )
