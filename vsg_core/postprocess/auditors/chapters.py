...
class ChaptersAuditor(BaseAuditor):
    ...
    def _verify_processed_chapters(self, final_chapters_xml: str | None) -> int:
        ...
        try:
            ...
            processed_nsmap, processed_prefix = self._get_xpath_and_nsmap(
                processed_root
            )
            final_nsmap, final_prefix = self._get_xpath_and_nsmap(final_root)

            processed_atoms = processed_root.xpath(
                f"//{processed_prefix}ChapterAtom", namespaces=processed_nsmap
            )
            final_atoms = final_root.xpath(
                f"//{final_prefix}ChapterAtom", namespaces=final_nsmap
            )
    ...
    def _compare_timestamps(
        ...
        for i, (proc_atom, final_atom) in enumerate(
            ...
            # Compare start times
            proc_start = proc_atom.find(
                f"{proc_prefix}ChapterTimeStart", namespaces=proc_nsmap
            )
            final_start = final_atom.find(
                f"{final_prefix}ChapterTimeStart", namespaces=final_nsmap
            )
            ...
            proc_end = proc_atom.find(
                f"{proc_prefix}ChapterTimeEnd", namespaces=proc_nsmap
            )
            final_end = final_atom.find(
                f"{final_prefix}ChapterTimeEnd", namespaces=final_nsmap
            )
    ...
    def _extract_chapters(self, file_path) -> str | None:
        ...
        try:
            ...
            self.log(
                f"[WARNING] Could not extract chapters from {Path(file_path).name}: {e}"
            )
        return None
