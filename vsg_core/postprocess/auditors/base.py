...
class BaseAuditor:
    ...
    def _get_metadata(self, file_path: str, tool: str) -> dict | None:
        ...
        cache = (
            self._source_mkvmerge_cache
            if tool == "mkvmerge"
            else self._source_ffprobe_cache
        )
        ...
        try:
            if tool == "mkvmerge":
                ...
                if max_delay_ms > 5000:
                    self.log(
                        f"[INFO] Detected large timestamp offset ({max_delay_ms:.0f}ms) in {Path(file_path).name}. Using enhanced ffprobe parameters."
                    )
    ...
    def _has_hdr_metadata(self, stream: dict) -> bool:
        ...
        for side_data in stream.get("side_data_list", []):
            if side_data.get("side_data_type") in [
