    ...
    def run(
        ...
        for plan_item in video_items:
            ...
            if source_video.get("width") != actual_video.get(
                ...
                self.log(
                    f"[WARNING] Resolution mismatch! Source: {source_video.get('width')}x{source_video.get('height')}, "
            ...
            if source_field_order != actual_field_order:
                self.log(
                    f"[WARNING] Field order mismatch! Source: '{source_field_order}', Output: '{actual_field_order}'"
                )
                if (
                    ...
                    self.log(
                        "         CRITICAL: Interlaced content marked as progressive! This will cause playback issues."
                    )
                issues += 1
            ...
            if source_mkv_data:
                ...
                if source_mkv_video and actual_mkv_video:
                    ...
                    if source_mkv_field != actual_mkv_field:
                        self.log(
                            f"[WARNING] MKV field_order flag mismatch! Source: {source_mkv_field}, Output: {actual_mkv_field}"
                        )
                        if source_mkv_field and not actual_mkv_field:
                            self.log(
                                "         Field order information was lost! May cause deinterlacing issues."
                            )
                        issues += 1
                    ...
                    if source_stereo != actual_stereo:
                        self.log(
                            f"[WARNING] Stereo mode (3D) mismatch! Source: {source_stereo}, Output: {actual_stereo}"
                        )
                        issues += 1
            ...
            if source_color_transfer and source_color_transfer != actual_color_transfer:
                self.log(
                    f"[WARNING] Color transfer mismatch! Source: '{source_color_transfer}', Output: '{actual_color_transfer}'"
                )
            ...
            for attr, label in [
                ...
                if source_video.get(attr) != actual_video.get(attr):
                    self.log(
                        f"[WARNING] {label} mismatch! Source: '{source_video.get(attr)}', Output: '{actual_video.get(attr)}'"
                    )
                    issues += 1
            ...
            if source_video.get("pix_fmt") != actual_video.get("pix_fmt"):
                self.log(
                    f"[WARNING] Pixel format mismatch! Source: '{source_video.get('pix_fmt')}', Output: '{actual_video.get('pix_fmt')}'"
                )
            ...
            if source_dar != actual_dar:
                self.log(
                    f"[WARNING] Display aspect ratio mismatch! Source: '{source_dar}', Output: '{actual_dar}'"
                )
                issues += 1
            ...
            if source_fps != actual_fps:
                self.log(
                    f"[WARNING] Frame rate mismatch! Source: '{source_fps}', Output: '{actual_fps}'"
                )
                issues += 1
            ...
            if source_mastering and not actual_mastering:
                self.log(
                    "[WARNING] Mastering display metadata (HDR10) was lost during merge!"
                )
            ...
            if source_cll and not actual_cll:
                self.log(
                    "[WARNING] Content light level metadata (MaxCLL/MaxFALL) was lost during merge!"
                )
                self.log(
                    f"         Lost data: MaxCLL={source_cll.get('max_content')}, MaxFALL={source_cll.get('max_average')}"
                )
                issues += 1
                ...
                if source_cll.get("max_content") != actual_cll.get(
                    ...
                    self.log(
                        f"[WARNING] Content light level mismatch! "
