...
"""
Handles delay discovery through audio cross-correlation or VideoDiff.
"""
...
def get_audio_stream_index(mkv_path: str, runner: CommandRunner, tool_paths: dict, language: Optional[str]) -> Optional[int]:
...
def extract_audio_chunk(
    source_file: str, output_wav: str, start_time: float, duration: float,
    runner: CommandRunner, tool_paths: dict, stream_index: int
):
...
def find_audio_delay(ref_wav: str, sec_wav: str, log_callback) -> Tuple[Optional[int], float, Optional[float]]:
    ...
    try:
        ...
        log_callback(f'Error in find_audio_delay: {e}')
        return None, 0.0, None
...
def run_audio_correlation(
    ...
    out = runner.run([ffprobe_path, '-v', 'error', '-show_entries', 'format=duration', '-of', 'csv=p=0', str(ref_file)], tool_paths)
    try:
    ...
    for i, start_time in enumerate(starts, 1):
        ...
        try:
            ok1 = extract_audio_chunk(ref_file, str(tmp1), start_time, chunk_dur, runner, tool_paths, idx1)
            ok2 = extract_audio_chunk(target_file, str(tmp2), start_time, chunk_dur, runner, tool_paths, idx2)

            if ok1 and ok2:
                delay, match, raw = find_audio_delay(str(tmp1), str(tmp2), runner._log_message)
...
def run_videodiff(ref_file: str, target_file: str, config: dict, runner: CommandRunner, tool_paths: dict) -> Tuple[int, float]:
    ...
    kind, s_val, err_val = match.groups()
    seconds = float(s_val)
