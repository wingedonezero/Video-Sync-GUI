...
def analyze_sync_stability(
    ...
    stepping_clusters: list[dict[str, Any]] | None = None,
...
def _analyze_uniform(
    ...
    if log:
        if variance_detected:
            ...
            log(
                f"  - Max variance: {max_variance:.4f}ms (threshold: {variance_threshold}ms)"
            )
            ...
            log(
                f"[Sync Stability] {source_key}: OK - consistent results (variance: {max_variance:.4f}ms)"
            )
...
def _analyze_with_clusters(
    ...
    source_key: str,
    log: Callable[[str], None] | None,
    stepping_clusters: list[dict[str, Any]],
    variance_threshold: float,
    outlier_mode: str,
    outlier_threshold: float,
) -> SyncStabilityIssue:
