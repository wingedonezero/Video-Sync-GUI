...
def _build_track_description(track: dict) -> str:
    ...
    if ttype == "video":
        ...
        if color_transfer == "smpte2084":
            detail_order.append("HDR")
        elif color_transfer == "arib-std-b67":
            detail_order.append("HLG")

        side_data = ffprobe_info.get("side_data_list", [])
        if any(
            s.get("side_data_type") == "DOVI configuration record" for s in side_data
        ):
...
def get_stream_info(
    mkv_path: str, runner: CommandRunner, tool_paths: dict
) -> dict[str, Any] | None:
...
def get_stream_info_with_delays(
    mkv_path: str, runner: CommandRunner, tool_paths: dict
) -> dict[str, Any] | None:
...
def _get_detailed_stream_info(
    filepath: str, runner: CommandRunner, tool_paths: dict
) -> dict[int, dict]:
...
def _ext_for_codec(ttype: str, codec_id: str) -> str:
    ...
    if ttype == "video":
        if "V_MPEGH/ISO/HEVC" in cid:
            return "h265"
        if "V_MPEG4/ISO/AVC" in cid:
            return "h264"
        if "V_MPEG" in cid:
            return "mpg"
        if "V_VP9" in cid:
    ...
    if ttype == "audio":
        ...
        return "bin"
    if ttype == "subtitles":
        if "S_TEXT/ASS" in cid:
            return "ass"
        if "S_TEXT/SSA" in cid:
            return "ssa"
        if "S_TEXT/UTF8" in cid:
            return "srt"
        if "S_HDMV/PGS" in cid:
            return "sup"
        if "S_VOBSUB" in cid:
            return "sub"
        return "sub"
...
def extract_tracks(
    ...
    tool_paths: dict,
    role: str,
    specific_tracks: list[int] | None = None,
) -> list[dict[str, Any]]:
    """
    ...
    if specs:
        ...
        if result is None:
            ...
            for spec in specs:
                ...
                track_info = next(
                    (t for t in tracks_to_extract if t["id"] == tid), None
                )
                ...
                if out_path.exists() and out_path.stat().st_size > 0:
                    ...
                    status = (
                        "not created" if not out_path.exists() else "empty (0 bytes)"
                    )
            ...
            if successful_tracks:
                error_msg += (
                    f"Successfully extracted ({len(successful_tracks)} tracks):\n"
                )
            ...
            if failed_tracks:
                for track_line in failed_tracks[:3]:
                    ...
                    error_msg += (
                        f'     mkvextract "{mkv}" tracks {tid}:test_track_{tid}.bin\n'
                    )
        ...
        for spec in specs:
            ...
            if not out_path.exists():
                track_name = (
                    track_info.get("name", f"Track {tid}")
                    if track_info
                    else f"Track {tid}"
                )
                verification_failed.append(
                    f"  • {track_name} (ID {tid}): File not created"
                )
                ...
                verification_failed.append(
                    f"  • {track_name} (ID {tid}): File is empty (0 bytes)"
                )
    ...
    for job in ffmpeg_jobs:
        ...
        runner._log_message(
            f"[{role}] Extracting A_MS/ACM track '{track_name}' (ID {job['tid']})..."
        )
        ...
        if runner.run(copy_cmd, tool_paths) is None:
            runner._log_message(
                f"[{role}] Stream copy refused. Falling back to PCM ({job['pcm']})..."
            )
            ...
            if runner.run(pcm_cmd, tool_paths) is None:
                ...
                error_msg += (
                    f"  2. Try: mkvextract \"{mkv}\" tracks {job['tid']}:test.wav\n"
                )
...
def get_track_info_for_dialog(
    sources: dict[str, str], runner: CommandRunner, tool_paths: dict
) -> dict[str, list[dict]]:
    ...
    for source_key, filepath in sources.items():
        ...
        ffprobe_streams_by_type = {
            "video": sorted(
                [s for s in ffprobe_details.values() if s.get("codec_type") == "video"],
                key=lambda s: s["index"],
            ),
            "audio": sorted(
                [s for s in ffprobe_details.values() if s.get("codec_type") == "audio"],
                key=lambda s: s["index"],
            ),
            "subtitles": sorted(
                [
                    s
                    for s in ffprobe_details.values()
        ...
        for track in mkvmerge_info.get("tracks", []):
            ...
            record = {
                ...
                "id": track["id"],
                "type": track["type"],
                "codec_id": props.get("codec_id", "N/A"),
                "lang": props.get("language", "und"),
                "name": props.get("track_name", ""),
                "audio_channels": (
                    props.get("audio_channels", "") if track["type"] == "audio" else ""
                ),
