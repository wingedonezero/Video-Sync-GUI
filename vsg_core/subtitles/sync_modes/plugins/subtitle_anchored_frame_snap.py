...
class SubtitleAnchoredFrameSnapSync(SyncPlugin):
    ...
    def apply(
        ...
        **kwargs
        ...
        # Open video readers
        try:
            source_reader = VideoReader(source_video, runner, use_vapoursynth=config.get('sub_anchor_use_vapoursynth', True), temp_dir=temp_dir)
            target_reader = VideoReader(target_video, runner, use_vapoursynth=config.get('sub_anchor_use_vapoursynth', True), temp_dir=temp_dir)
