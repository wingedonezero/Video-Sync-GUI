...
def multiply_font_size(
    subtitle_path: str, multiplier: float, runner: CommandRunner
) -> bool:
    sub_path = Path(subtitle_path)
    if sub_path.suffix.lower() not in [".ass", ".ssa"] or multiplier == 1.0:
        return False
    runner._log_message(
        f"[Font Size] Applying {multiplier:.2f}x size multiplier to {sub_path.name}."
    )
    try:
        ...
        if modified > 0:
            ...
            return True
        runner._log_message(
            f"[Font Size] WARN: No style definitions found to modify in {sub_path.name}."
        )
        return False
    except Exception as e:
        runner._log_message(
            f"[Font Size] ERROR: Could not process {sub_path.name}: {e}"
        )
        return False
