...
def run_pal_correction(ctx: Context, runner: CommandRunner) -> Context:
    ...
    for analysis_track_key, flag_info in ctx.pal_drift_flags.items():
        ...
        if not target_items:
            runner._log_message(f"[PALCorrector] Could not find target audio tracks for {source_key} in the layout. Skipping.")
            continue

        runner._log_message(f"[PALCorrector] Applying PAL speed correction to {len(target_items)} track(s) from {source_key}...")
        ...
        for target_item in target_items:
            ...
            target_item.track = Track(
                ...
                props=StreamProps(
                    codec_id="FLAC",
                    lang=original_props.lang,
                    name=f"{original_props.name} (PAL Corrected)" if original_props.name else "PAL Corrected"  # ENHANCED: Clearer name
                )
            )
