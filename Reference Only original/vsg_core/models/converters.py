    ...
    for sel in manual_layout or []:
        ...
        realized.append(
            PlanItem(
                ...
                source_track_id=sel.get("source_track_id"),
                filter_config=sel.get("filter_config"),
                original_style_list=sel.get("original_style_list", []),
            )
        )
    return realized


def signature_for_auto_apply(
    track_info: dict[str, list[Track]], strict: bool = False
) -> Counter:
    """
