...
def check_if_rebasing_is_needed(
    ...
    try:
        ...
        if out and float(out.strip()) > 0.01:
            runner._log_message(
                f"[Finalize] First video timestamp is {float(out.strip()):.3f}s. Rebasing is required."
            )
            return True
...
def finalize_merged_file(
    ...
    if not ffmpeg_ok:
        log(
            "[WARNING] Timestamp normalization with FFmpeg failed. The original merged file will be used."
        )
