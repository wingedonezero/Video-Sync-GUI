...
from vsg.api import analyze_and_plan
    ...
    try:
        while True:
            ...
            if dpg.does_item_exist("log_scroller"):
                ...
                if dpg.does_item_exist("log_child") and CONFIG.get(
                    "log_autoscroll", True
                ):
                    try:
...
def sync_config_from_ui():
    ...
    CONFIG.update(
        {
            "output_folder": dpg.get_value("out_input")
            or CONFIG.get("output_folder", str(SCRIPT_DIR / "sync_output")),
            "temp_root": dpg.get_value("temp_input")
            or CONFIG.get("temp_root", str(SCRIPT_DIR / "temp_work")),
            "analysis_mode": dpg.get_value("mode_combo")
            or CONFIG.get("analysis_mode", "Audio Correlation"),
            "workflow": dpg.get_value("workflow_combo")
            or CONFIG.get("workflow", "Analyze & Merge"),
            "scan_chunk_count": (
                int(dpg.get_value("chunks_input"))
                if dpg.does_item_exist("chunks_input")
                else CONFIG.get("scan_chunk_count", 10)
            ),
            "scan_chunk_duration": (
                int(dpg.get_value("chunkdur_input"))
                if dpg.does_item_exist("chunkdur_input")
                else CONFIG.get("scan_chunk_duration", 15)
            ),
            "min_match_pct": (
                float(dpg.get_value("thresh_input"))
                if dpg.does_item_exist("thresh_input")
                else CONFIG.get("min_match_pct", 5.0)
            ),
            "videodiff_error_min": (
                float(dpg.get_value("vd_err_min"))
                if dpg.does_item_exist("vd_err_min")
                else CONFIG.get("videodiff_error_min", 0.0)
            ),
            "videodiff_error_max": (
                float(dpg.get_value("vd_err_max"))
                if dpg.does_item_exist("vd_err_max")
                else CONFIG.get("videodiff_error_max", 100.0)
            ),
            "swap_subtitle_order": (
                bool(dpg.get_value("swapsec_chk"))
                if dpg.does_item_exist("swapsec_chk")
                else CONFIG.get("swap_subtitle_order", False)
            ),
            "rename_chapters": (
                bool(dpg.get_value("chaprename_chk"))
                if dpg.does_item_exist("chaprename_chk")
                else CONFIG.get("rename_chapters", False)
            ),
            "match_jpn_secondary": (
                bool(dpg.get_value("jpnsec_chk"))
                if dpg.does_item_exist("jpnsec_chk")
                else CONFIG.get("match_jpn_secondary", True)
            ),
            "match_jpn_tertiary": (
                bool(dpg.get_value("jpnter_chk"))
                if dpg.does_item_exist("jpnter_chk")
                else CONFIG.get("match_jpn_tertiary", True)
            ),
            "apply_dialog_norm_gain": (
                bool(dpg.get_value("dialnorm_chk"))
                if dpg.does_item_exist("dialnorm_chk")
                else CONFIG.get("apply_dialog_norm_gain", False)
            ),
            "first_sub_default": (
                bool(dpg.get_value("firstsubdef_chk"))
                if dpg.does_item_exist("firstsubdef_chk")
                else CONFIG.get("first_sub_default", True)
            ),
            "videodiff_path": (
                dpg.get_value("vdiff_input")
                if dpg.does_item_exist("vdiff_input")
                else CONFIG.get("videodiff_path", "")
            ),
            "snap_chapters": (
                bool(dpg.get_value("snap_chapters_chk"))
                if dpg.does_item_exist("snap_chapters_chk")
                else CONFIG.get("snap_chapters", False)
            ),
            "snap_mode": (
                dpg.get_value("snap_mode_opt")
                if dpg.does_item_exist("snap_mode_opt")
                else CONFIG.get("snap_mode", "previous")
            ),
            "snap_threshold_ms": (
                int(dpg.get_value("snap_threshold_ms"))
                if dpg.does_item_exist("snap_threshold_ms")
                else CONFIG.get("snap_threshold_ms", 250)
            ),
            "snap_starts_only": (
                bool(dpg.get_value("snap_starts_only_chk"))
                if dpg.does_item_exist("snap_starts_only_chk")
                else CONFIG.get("snap_starts_only", True)
            ),
        }
    )
    Path(CONFIG["output_folder"]).mkdir(parents=True, exist_ok=True)
            ...
            if compact:
                if line.startswith("Progress: "):
                    ...
                    if pct is not None and (
                        last_prog < 0 or pct >= last_prog + prog_step or pct == 100
                    ):
...
def extract_tracks(
    mkv: str, temp_dir: str, logger, role: str, audio=True, subs=True, all_tracks=False
) -> List[Dict[str, Any]]:
    ...
    for tr in info.get("tracks", []):
        ...
        want = (
            all_tracks
            or (audio and ttype == "audio")
            or (subs and ttype == "subtitles")
            or (ttype == "video" and all_tracks)
        )
        if not want:
            continue
...
def rename_chapters_xml(
    ref_mkv: str, temp_dir: str, logger, shift_ms: int = 0
) -> Optional[str]:
    ...
    try:
        ...
        try:
            ...
            def _parse_hhmmss_ns(t: str):
                ...
                if "." in rest:
                    ...
                    ss, frac = (rest, "000000000")
                total_ns = (int(hh) * 3600 + int(mm) * 60 + int(ss)) * 1000000000 + int(
                    frac
                )
                return total_ns
        ...
        try:
            _log(
                logger,
                "[Chapters] Snap: enabled=%s mode=%s thr=%sms starts_only=%s"
                % (
                    CONFIG.get("snap_chapters"),
                    CONFIG.get("snap_mode"),
                    CONFIG.get("snap_threshold_ms"),
                    CONFIG.get("snap_starts_only"),
                ),
            )
            if CONFIG.get("snap_chapters", False):
                ...
                (
                    _log(logger, f"[Chapters] Snap sidecar path: {sidecar}")
                    if CONFIG.get("chapter_snap_verbose", False)
                    else None
                )
                try:
                    ...
                    _log(
                        logger,
                        "[Chapters] Snap result: "
                        + ", ".join(parts)
                        + f" (kfs={len(kfs)}, mode={mode_}, thr={int(CONFIG.get('snap_threshold_ms', 250))}ms)",
                    )
                    if moved == 0:
                ...
                try:
                    ...
                    _log(
                        logger,
                        "[Chapters] Snap classify: "
                        + ", ".join(parts)
                        + f" (mode={mode_}, thr={int(CONFIG.get('snap_threshold_ms', 250))}ms)",
                    )
                    if CONFIG.get("chapter_snap_verbose", False) and near_nochange:
                        _log(
                            logger,
                            "[Chapters] note: near_nochange > 0 means start was inside threshold but unchanged (edge case).",
                        )
                except Exception as _e:
                ...
                try:
                    ...
                    if _moved:
                        _log(
                            logger,
                            f"[Chapters] Snap summary: moved {len(_moved)} starts; min={min(_moved)}ms max={max(_moved)}ms",
                        )
                    else:
...
def get_audio_stream_index(
    file_path: str, logger, language: Optional[str]
) -> Optional[int]:
...
def run_audio_correlation_workflow(
    file1: str,
    file2: str,
    logger,
    chunks: int,
    chunk_dur: int,
    match_lang: Optional[str],
    role_tag: str,
):
    ...
    _log(
        logger,
        f"Chosen streams -> ref a:{idx1}, target a:{idx2} (prefer='{match_lang or 'first'}')",
    )
    ...
    for i, st in enumerate(starts, 1):
        ...
        tmp1 = (
            Path(CONFIG["temp_root"]) / f"wav_ref_{Path(file1).stem}_{int(st)}_{i}.wav"
        )
        tmp2 = (
            Path(CONFIG["temp_root"])
            / f"wav_{role_tag}_{Path(file2).stem}_{int(st)}_{i}.wav"
        )
        try:
            ...
            if ok1 and ok2:
                ...
                if delay is not None:
                    results.append(
                        {"delay": delay, "match": match, "raw_delay": raw, "start": st}
                    )
                    _log(
                        logger,
                        f"Chunk @{int(st)}s -> Delay {delay:+} ms (Match {match:.2f}%) Raw {raw:.6f}s",
                    )
        finally:
            for p in (tmp1, tmp2):
                try:
...
def summarize_plan(plan_json, output_file, chapters_xml_path, attachments):
    ...
    lines.append(
        f"Delays: secondary={delays.get('secondary_ms', 0)} ms, tertiary={delays.get('tertiary_ms', 0)} ms"
    )
    ...
    for idx, trk in enumerate(plan):
        ...
        lines.append(
            f"  [{idx}] {ttype:9s} | group={grp:3s} | lang={lang:3s} | name='{name}' | codec='{codec}'"
        )
...
def build_mkvmerge_tokens(
    plan_json, output_file, chapters_xml_path, attachments, track_order_str=None
):
    ...
    audio_langs = [
        (t.get("lang") or "").lower() for t in plan if t.get("type") == "audio"
    ]
    ...
    for idx, trk in enumerate(plan):
        ...
        perfile = []
        perfile += _tokens_for_track(
            trk, trk.get("from_group"), delays, default_flag=default_flag
        )
...
def run_videodiff(
    ref: str, target: str, logger, videodiff_path: Path | str
) -> Tuple[int, float]:
    ...
    m = re.search(
        "(itsoffset|ss)\\s*:\\s*(-?\\d+(?:\\.\\d+)?)s.*?error:\\s*([0-9.]+)",
        last_line,
        flags=re.IGNORECASE,
    )
    if not m:
    ...
    _log(
        logger,
        f"[VideoDiff] final -> {kind.lower()} {seconds:.5f}s, error {err_val:.2f}  =>  delay {delay_ms:+} ms",
    )
...
# Delegate analysis + delay normalization to vsg.api.analyze_and_plan
set_status("Analyzing…")
set_progress(0.0)

# Determine analysis mode and per-track language preferences (JPN matching)
mode = CONFIG.get("analysis_mode", "Audio Correlation")
prefer_lang_sec = "jpn" if CONFIG.get("match_jpn_secondary", True) else None
prefer_lang_ter = "jpn" if CONFIG.get("match_jpn_tertiary", True) else None
...
vd_min = float(CONFIG.get("videodiff_error_min", 0.0))
vd_max = float(CONFIG.get("videodiff_error_max", 100.0))

# videodiff path is computed by caller; pass it along
delay_sec, delay_ter, delays = analyze_and_plan(
    ref_file=ref_file,
    sec_file=sec_file,
    ter_file=ter_file,
    mode=mode,
    prefer_lang_sec=prefer_lang_sec,
    prefer_lang_ter=prefer_lang_ter,
    chunks=chunks,
    chunk_dur=chunk_dur,
    min_match_pct=min_match_pct,
    videodiff_path=str(videodiff_path) if videodiff_path else "",
    videodiff_error_min=vd_min,
    videodiff_error_max=vd_max,
    logger=logger,
)

# Update UI delay fields (if present)
...
try:
    ...
    if CONFIG.get("rename_chapters", False):
        chapters_xml = rename_chapters_xml(
            ref_file,
            str(job_temp),
            logger,
    ...
    # Extract streams for plan construction
    ...
    # Optional swap: first two secondary subtitles (before default selection)
    if CONFIG.get("swap_subtitle_order") and sec_tracks:
    ...
    if CONFIG.get("log_merge_summary", False):
        try:
            from vsg.logging import log_merge_summary_from_tokens  # optional module
    ...
    if merge_ok or CONFIG.get("workflow") == "Analyze Only":
        try:
...
def _snap_chapter_times_inplace(
    ...
    _log(
        logger,
        f"[Chapters] Snapped chapters: {changed}/{len(atoms)} (mode={mode}, threshold={threshold_ms} ms, starts_only={starts_only})",
    )
    if sidecar_path:
        try:
...
def do_analyze_only(sender, app_data, user_data):
    threading.Thread(
        target=worker_run_jobs, kwargs={"and_merge": False}, daemon=True
    ).start()


def do_analyze_and_merge(sender, app_data, user_data):
    threading.Thread(
        target=worker_run_jobs, kwargs={"and_merge": True}, daemon=True
    ).start()
...
def worker_run_jobs(and_merge=False):
    ...
    vd_path = (
        Path(vd_cfg)
        if vd_cfg
        else Path(shutil.which("videodiff") or SCRIPT_DIR / "videodiff")
    )
    results = []
...
def discover_jobs(ref_path, sec_path, ter_path):
    ...
    if ref.is_file():
        return [
            (
                str(ref),
                str(sec) if sec and sec.is_file() else None,
                str(ter) if ter and ter.is_file() else None,
            )
        ]
    if sec and sec.is_file() or (ter and ter.is_file()):
        raise RuntimeError(
            "If Reference is a folder, Secondary/Tertiary must be folders too."
        )
    jobs = []
...
def build_ui():
    ...
    try:
        with dpg.theme(tag="input_theme"):
            with dpg.theme_component(dpg.mvInputText):
                dpg.add_theme_style(
                    dpg.mvStyleVar_FramePadding, 10, 8, category=dpg.mvThemeCat_Core
                )
    except Exception:
        pass
    try:
        with dpg.theme(tag="control_theme"):
            with dpg.theme_component(dpg.mvButton):
                dpg.add_theme_style(
                    dpg.mvStyleVar_FramePadding, 12, 10, category=dpg.mvThemeCat_Core
                )
            with dpg.theme_component(dpg.mvCombo):
                dpg.add_theme_style(
                    dpg.mvStyleVar_FramePadding, 10, 9, category=dpg.mvThemeCat_Core
                )
            with dpg.theme_component(dpg.mvProgressBar):
                dpg.add_theme_style(
                    dpg.mvStyleVar_FramePadding, 6, 6, category=dpg.mvThemeCat_Core
                )
    except Exception:
        pass
    ...
    with dpg.window(tag="main_window", label=APP_NAME, width=1180, height=780):
        ...
        with dpg.group(horizontal=True):
            dpg.add_button(
                label="Storage…",
                callback=lambda: dpg.configure_item("storage_modal", show=True),
            )
            dpg.add_button(
            ...
            dpg.add_button(
                ...
                callback=lambda: dpg.configure_item("global_modal", show=True),
            )
        ...
        with dpg.group(horizontal=True):
            ...
            dpg.add_input_text(
                tag="ref_input", label="", width=900, multiline=False, height=40
            )
            if INPUT_FONT_ID:
                dpg.bind_item_font("ref_input", INPUT_FONT_ID)
            _safe_bind_input_enhancements("ref_input")
            dpg.add_button(
                label="Browse…", callback=lambda: dpg.show_item("file_dialog_ref")
            )
        with dpg.group(horizontal=True):
            dpg.add_text("Secondary")
            dpg.add_input_text(
                tag="sec_input", label="", width=900, multiline=False, height=40
            )
            if INPUT_FONT_ID:
                dpg.bind_item_font("sec_input", INPUT_FONT_ID)
            _safe_bind_input_enhancements("sec_input")
            dpg.add_button(
                label="Browse…", callback=lambda: dpg.show_item("file_dialog_sec")
            )
        with dpg.group(horizontal=True):
            dpg.add_text("Tertiary")
            dpg.add_input_text(
                tag="ter_input", label="", width=900, multiline=False, height=40
            )
            if INPUT_FONT_ID:
                dpg.bind_item_font("ter_input", INPUT_FONT_ID)
            _safe_bind_input_enhancements("ter_input")
            dpg.add_button(
                label="Browse…", callback=lambda: dpg.show_item("file_dialog_ter")
            )
        dpg.add_separator()
        ...
        with dpg.group(horizontal=True):
            ...
            dpg.add_combo(
                tag="workflow_combo",
                items=["Analyze & Merge", "Analyze Only"],
                default_value=CONFIG["workflow"],
                width=260,
            )
            ...
            dpg.add_combo(
                ...
                default_value=CONFIG["analysis_mode"],
                width=260,
            )
        ...
        with dpg.group(horizontal=True):
            dpg.add_button(
                tag="btn_analyze_only",
                label="Analyze Only",
                callback=do_analyze_only,
                width=150,
                height=36,
            )
            _bind_control_theme("btn_analyze_only")
            dpg.add_button(
                tag="btn_analyze_merge",
                label="Analyze & Merge",
                callback=do_analyze_and_merge,
                width=170,
                height=36,
            )
        ...
        with dpg.child_window(
            tag="log_child", width=-1, height=320, horizontal_scrollbar=True
        ):
        ...
        with dpg.file_dialog(
            tag="dir_dialog_out",
            label="Pick Output Directory",
            callback=on_pick_dir,
            user_data="out_input",
            width=700,
            height=400,
        ...
        with dpg.file_dialog(
            tag="dir_dialog_temp",
            label="Pick Temp Directory",
            callback=on_pick_dir,
            user_data="temp_input",
            width=700,
            height=400,
            directory_selector=True,
            show=False,
            ...
            pass
        with dpg.file_dialog(
            tag="file_dialog_vdiff",
            label="Pick videodiff binary",
            callback=on_pick_file,
            user_data="vdiff_input",
            width=700,
            height=400,
            directory_selector=False,
            show=False,
        ...
        with dpg.window(
            ...
            dpg.add_text(
                "Set directories for temporary work and final outputs; optionally set videodiff path"
            )
            dpg.add_separator()
            with dpg.group(horizontal=True):
                ...
                dpg.add_input_text(
                    tag="temp_input",
                    label="",
                    default_value=CONFIG["temp_root"],
                    width=600,
                    multiline=False,
                    height=40,
                )
                ...
                dpg.add_button(
                    label="Browse…", callback=lambda: dpg.show_item("dir_dialog_temp")
                )
            with dpg.group(horizontal=True):
                ...
                dpg.add_input_text(
                    tag="out_input",
                    label="",
                    default_value=CONFIG["output_folder"],
                    width=600,
                    multiline=False,
                    height=40,
                )
                ...
                dpg.add_button(
                    label="Browse…", callback=lambda: dpg.show_item("dir_dialog_out")
                )
            with dpg.group(horizontal=True):
                ...
                dpg.add_input_text(
                    tag="vdiff_input",
                    label="",
                    default_value=CONFIG.get("videodiff_path", ""),
                    width=600,
                    multiline=False,
                    height=40,
                )
                ...
                dpg.add_button(
                    label="Browse…", callback=lambda: dpg.show_item("file_dialog_vdiff")
                )
            dpg.add_separator()
            with dpg.group(horizontal=True):
                dpg.add_button(
                    ...
                    callback=lambda s, a, u: (
                        ui_save_settings(),
                        dpg.configure_item("storage_modal", show=False),
                    ),
                )
                dpg.add_button(
                    label="Cancel",
                    callback=lambda: dpg.configure_item("storage_modal", show=False),
                )
        with dpg.window(
            tag="global_modal",
            label="Global Options",
            ...
            with dpg.group(horizontal=True):
                ...
                dpg.add_checkbox(
                    tag="swapsec_chk",
                    label="",
                    default_value=bool(CONFIG["swap_subtitle_order"]),
                )
            with dpg.group(horizontal=True):
                ...
                dpg.add_checkbox(
                    tag="chaprename_chk",
                    label="",
                    default_value=bool(CONFIG["rename_chapters"]),
                )
            with dpg.group(horizontal=True):
                ...
                dpg.add_checkbox(
                    tag="jpnsec_chk",
                    label="",
                    default_value=bool(CONFIG["match_jpn_secondary"]),
                )
            with dpg.group(horizontal=True):
                ...
                dpg.add_checkbox(
                    tag="jpnter_chk",
                    label="",
                    default_value=bool(CONFIG["match_jpn_tertiary"]),
                )
            with dpg.group(horizontal=True):
                ...
                dpg.add_checkbox(
                    tag="dialnorm_chk",
                    label="",
                    default_value=bool(CONFIG["apply_dialog_norm_gain"]),
                )
            with dpg.group(horizontal=True):
                ...
                dpg.add_checkbox(
                    tag="firstsubdef_chk",
                    label="",
                    default_value=bool(CONFIG.get("first_sub_default", True)),
                )
            dpg.add_separator()
            with dpg.group(horizontal=True):
                dpg.add_button(
                    ...
                    callback=lambda s, a, u: (
                        ui_save_settings(),
                        dpg.configure_item("global_modal", show=False),
                    ),
                )
                dpg.add_button(
                    label="Cancel",
                    callback=lambda: dpg.configure_item("global_modal", show=False),
                )
            dpg.add_separator()
            ...
            with dpg.group(horizontal=True):
                ...
                dpg.add_combo(
                    tag="snap_mode_opt",
                    items=["previous", "nearest"],
                    default_value=CONFIG.get("snap_mode", "previous"),
                    width=140,
                )
            with dpg.group(horizontal=True):
                dpg.add_text("Max snap distance (ms)")
                dpg.add_input_int(
                    tag="snap_threshold_ms",
                    default_value=int(CONFIG.get("snap_threshold_ms", 250)),
                    min_value=0,
                    width=120,
                )
            with dpg.group(horizontal=True):
        ...
        with dpg.window(
            ...
            with dpg.group(horizontal=True):
                ...
                dpg.add_input_int(
                    tag="chunks_input",
                    label="",
                    default_value=int(CONFIG["scan_chunk_count"]),
                    min_value=1,
                    max_value=50,
                    width=100,
                )
                ...
                dpg.add_input_int(
                    tag="chunkdur_input",
                    label="",
                    default_value=int(CONFIG["scan_chunk_duration"]),
                    min_value=2,
                    max_value=120,
                    width=120,
                )
                ...
                dpg.add_input_float(
                    tag="thresh_input",
                    label="",
                    default_value=float(CONFIG["min_match_pct"]),
                    width=120,
                )
            ...
            with dpg.group(horizontal=True):
                dpg.add_button(
                    ...
                    callback=lambda s, a, u: (
                        ui_save_settings(),
                        dpg.configure_item("analysis_modal", show=False),
                    ),
                )
                dpg.add_button(
                    label="Cancel",
                    callback=lambda: dpg.configure_item("analysis_modal", show=False),
                )
    Path(CONFIG["output_folder"]).mkdir(parents=True, exist_ok=True)
    Path(CONFIG["temp_root"]).mkdir(parents=True, exist_ok=True)
    try:
...
def _vsg_log_merge_summary_from_opts(logger, opts_path: str):
    ...
    for idx, item in enumerate(parsed.get("inputs", [])):
        ...
        details = []
        if lang:
            details.append(f"lang={lang}")
        if name:
            details.append(f"name={name}")
        if dflt:
            details.append(f"default={dflt}")
        if forc:
            details.append(f"forced={forc}")
        if sync:
            details.append(f"sync={sync_ms}")
