...
def merge_job(
    ...
    if sec_file:
        if CONFIG["analysis_mode"] == "VideoDiff":
            delay_sec, err_sec = run_videodiff(
                ref_file, sec_file, logger, videodiff_path
            )
            if err_sec < float(
                CONFIG.get("videodiff_error_min", 0.0)
            ) or err_sec > float(CONFIG.get("videodiff_error_max", 100.0)):
                raise RuntimeError(
            ...
            results = run_audio_correlation_workflow(
                ref_file,
                sec_file,
                logger,
                CONFIG["scan_chunk_count"],
                CONFIG["scan_chunk_duration"],
                lang,
                role_tag="sec",
            )
            ...
            if not best:
                raise RuntimeError(
                    "Audio analysis for Secondary yielded no valid result."
                )
            delay_sec = best["delay"]
        _log(logger, f"Secondary delay: {delay_sec} ms")
    if ter_file:
        if CONFIG["analysis_mode"] == "VideoDiff":
            delay_ter, err_ter = run_videodiff(
                ref_file, ter_file, logger, videodiff_path
            )
            if err_ter < float(
                CONFIG.get("videodiff_error_min", 0.0)
            ) or err_ter > float(CONFIG.get("videodiff_error_max", 100.0)):
                raise RuntimeError(
            ...
            if not best:
                raise RuntimeError(
                    "Audio analysis for Tertiary yielded no valid result."
                )
    ...
    _log(
        logger,
        f"[Delay] Raw group delays (ms): ref=0, sec={int(delay_sec or 0)}, ter={int(delay_ter or 0)}",
    )
    _log(logger, f"[Delay] Lossless global shift: +{int(global_shift)} ms")
    job_temp = (
        Path(CONFIG["temp_root"]) / f"job_{Path(ref_file).stem}_{int(time.time())}"
    )
    ...
    try:
        ...
        ref_tracks = extract_tracks(
            ref_file, str(job_temp), logger, role="ref", all_tracks=True
        )
        sec_tracks = (
            extract_tracks(
                sec_file, str(job_temp), logger, role="sec", audio=True, subs=True
            )
            if sec_file
            else []
        )
        ter_tracks = (
            extract_tracks(
                ter_file, str(job_temp), logger, role="ter", audio=False, subs=True
            )
            if ter_file
            else []
        )
        ter_atts = (
            extract_attachments(ter_file, str(job_temp), logger, role="ter")
            if ter_file
            else []
        )
        if CONFIG["swap_subtitle_order"] and sec_tracks:
            ...
            if len(only_subs) >= 2:
                i0, i1 = (
                    sec_tracks.index(only_subs[0]),
                    sec_tracks.index(only_subs[1]),
                )
        ...
        track_order_str, summary_lines = summarize_plan(
            plan, out_file, chapters_xml, ter_atts
        )
