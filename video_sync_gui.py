...
# === Fonts (loaded after create_context, bound before UI is built) ===
UI_FONT_ID = None
INPUT_FONT_ID = None
...
def load_fonts():
    ...
    try:
        with dpg.font_registry():
            ...
            # Pick a regular 18pt face for both inputs and UI
            picked = None
        ...
        if UI_FONT_ID:
            dpg.bind_font(UI_FONT_ID)  # same 18pt regular across the UI
...
CONFIG = {
    ...
    "min_match_pct": 5.0,
    "apply_dialog_norm_gain": False,         # AC-3/E-AC-3 only
    "videodiff_path": "",
    "first_sub_default": True,
    "videodiff_error_min": 0.0,
    "videodiff_error_max": 100.0,

    # Chapters → keyframe snapping (optional; OFF by default)
    "snap_chapters": False,
    "snap_mode": "previous",
...
# --------------------------
# Settings <-> UI sync helpers
# --------------------------
_UI_BIND = {
    "temp_root": "temp_input",
    "output_folder": "out_input",
    "videodiff_path": "vdiff_input",
    "workflow": "workflow_combo",
    "analysis_mode": "mode_combo",
    "swap_subtitle_order": "swapsec_chk",
    "rename_chapters": "chaprename_chk",
    "match_jpn_secondary": "jpnsec_chk",
    "match_jpn_tertiary": "jpnter_chk",
    "apply_dialog_norm_gain": "dialnorm_chk",
    "first_sub_default": "firstsubdef_chk",
    "snap_chapters": "snap_chapters_chk",
    "snap_mode": "snap_mode_opt",
    "snap_starts_only": "snap_starts_only_chk",
    "log_compact": "log_compact_chk",
...
def sync_config_from_ui():
    ...
    CONFIG.update({
        ...
        # Analysis settings
        "scan_chunk_count": int(dpg.get_value("chunks_input")) if dpg.does_item_exist("chunks_input") else CONFIG.get("scan_chunk_count", 10),
        "scan_chunk_duration": int(dpg.get_value("chunkdur_input")) if dpg.does_item_exist("chunkdur_input") else CONFIG.get("scan_chunk_duration", 15),
        "min_match_pct": float(dpg.get_value("thresh_input")) if dpg.does_item_exist("thresh_input") else CONFIG.get("min_match_pct", 5.0),
        "videodiff_error_min": float(dpg.get_value("vd_err_min")) if dpg.does_item_exist("vd_err_min") else CONFIG.get("videodiff_error_min", 0.0),
        "videodiff_error_max": float(dpg.get_value("vd_err_max")) if dpg.does_item_exist("vd_err_max") else CONFIG.get("videodiff_error_max", 100.0),
        # Global options
        "swap_subtitle_order": bool(dpg.get_value("swapsec_chk")) if dpg.does_item_exist("swapsec_chk") else CONFIG.get("swap_subtitle_order", False),
        "rename_chapters": bool(dpg.get_value("chaprename_chk")) if dpg.does_item_exist("chaprename_chk") else CONFIG.get("rename_chapters", False),
        "match_jpn_secondary": bool(dpg.get_value("jpnsec_chk")) if dpg.does_item_exist("jpnsec_chk") else CONFIG.get("match_jpn_secondary", True),
        "match_jpn_tertiary": bool(dpg.get_value("jpnter_chk")) if dpg.does_item_exist("jpnter_chk") else CONFIG.get("match_jpn_tertiary", True),
        "apply_dialog_norm_gain": bool(dpg.get_value("dialnorm_chk")) if dpg.does_item_exist("dialnorm_chk") else CONFIG.get("apply_dialog_norm_gain", False),
        "first_sub_default": bool(dpg.get_value("firstsubdef_chk")) if dpg.does_item_exist("firstsubdef_chk") else CONFIG.get("first_sub_default", True),
        "videodiff_path": dpg.get_value("vdiff_input") if dpg.does_item_exist("vdiff_input") else CONFIG.get("videodiff_path",""),
        # Chapter keyframe snapping
        "snap_chapters": bool(dpg.get_value("snap_chapters_chk")) if dpg.does_item_exist("snap_chapters_chk") else CONFIG.get("snap_chapters", False),
        "snap_mode": dpg.get_value("snap_mode_opt") if dpg.does_item_exist("snap_mode_opt") else CONFIG.get("snap_mode", "previous"),
        "snap_threshold_ms": int(dpg.get_value("snap_threshold_ms")) if dpg.does_item_exist("snap_threshold_ms") else CONFIG.get("snap_threshold_ms", 250),
        "snap_starts_only": bool(dpg.get_value("snap_starts_only_chk")) if dpg.does_item_exist("snap_starts_only_chk") else CONFIG.get("snap_starts_only", True),
    })
    Path(CONFIG["output_folder"]).mkdir(parents=True, exist_ok=True)
    Path(CONFIG["temp_root"]).mkdir(parents=True, exist_ok=True)
...
def run_command(cmd: List[str], logger) -> Optional[str]:
    ...
    try:
        ...
        if proc.returncode and proc.returncode > 1:
            _log(logger, f"[!] Command failed with exit code {proc.returncode}")
            return None
        return out
    except Exception as e:
...
def _ext_for_track(ttype: str, codec_id: str) -> str:
    ...
    if ttype == "video":
        if "V_MPEGH/ISO/HEVC" in cid or "HEVC" in cid or "H.265" in cid: return "h265"
        if "V_MPEG4/ISO/AVC" in cid or "H.264" in cid or "AVC" in cid:  return "h264"
        if "V_VP9" in cid: return "vp9"
        if "V_AV1" in cid or "AV1" in cid: return "av1"
        return "bin"
    if ttype == "audio":
        if "A_TRUEHD" in cid: return "thd"
        if "A_EAC3" in cid or "E-AC-3" in cid: return "eac3"
        ...
        if "A_PCM" in cid or "A_MS/ACM" in cid or "LPCM" in cid: return "wav"
        return "bin"
    if ttype == "subtitles":
        if "S_TEXT/ASS" in cid or "S_ASS" in cid: return "ass"
        if "S_TEXT/SSA" in cid or "S_SSA" in cid: return "ssa"
        if "S_TEXT/UTF8" in cid or "S_UTF8" in cid: return "srt"
        if "S_HDMV/PGS" in cid or "PGS" in cid: return "sup"
        if "S_VOBSUB" in cid: return "sub"
        return "sub"
    return "bin"

def extract_tracks(mkv: str, temp_dir: str, logger, role: str,
                   audio=True, subs=True, all_tracks=False) -> List[Dict[str, Any]]:
...
def _normalize_chapter_end_times(root, logger=None):
    ...
    def _parse_ns(t: str) -> int:
        ...
        hh, mm, rest = t.split(":")
        if "." in rest:
            ss, frac = rest.split("."); frac = (frac + "000000000")[:9]
        else:
    ...
    for i, (idx, atom, st_el, en_el, st_ns, en_ns) in enumerate(recs):
        if st_ns is None:  # nothing sensible to do
            continue
        # Determine next start (if any)
        next_start_ns = None
        if i+1 < len(recs):
            next_st = recs[i+1][4]
        ...
        # Desired end: clamp to next_start_ns (if exists)
        desired_en = en_ns
        # If there's a next start, we must not overlap it
        if next_start_ns is not None:
            if desired_en is None or desired_en > next_start_ns:
                desired_en = next_start_ns
        # Ensure strictly > start
        if desired_en is None or desired_en <= st_ns:
            desired_en = st_ns + 1_000_000  # +1 ms minimal span
...
def rename_chapters_xml(ref_mkv: str, temp_dir: str, logger, shift_ms: int = 0) -> Optional[str]:
    ...
    try:
        ...
        for i, atom in enumerate(root.findall(".//ChapterAtom"), 1):
            ...
            # Force ChapterLanguage="und" to avoid "en:" and make display neutral
            ET.SubElement(nd, "ChapterLanguage").text = "und"
        path = Path(temp_dir) / f"{Path(ref_mkv).stem}_chapters_mod.xml"
        
        # Shift ChapterTimeStart/ChapterTimeEnd by shift_ms (lossless-positive delay shift)
        try:
            shift_ns = int(shift_ms) * 1_000_000
            def _parse_hhmmss_ns(t: str):
                ...
                if "." in rest:
                    ...
                    ss, frac = rest, "000000000"
                total_ns = (int(hh)*3600 + int(mm)*60 + int(ss)) * 1_000_000_000 + int(frac)
                return total_ns
        ...
        try:
            ...
            if CONFIG.get("snap_chapters", False):
                ...
                _snap_chapter_times_inplace(root, kfs,
                                            bool(CONFIG.get("snap_starts_only", True)),
                                            str(CONFIG.get("snap_mode", "previous")),
                                            int(CONFIG.get("snap_threshold_ms", 250)),
                                            logger, sidecar, int(shift_ms))
                ...
                try:
                    ...
                    for r in recs:
                        ...
                        if d != 0:
                            moved += 1
                            deltas_ms.append(int(round(d/1_000_000)))
                        else:
                ...
                # Classification summary (moved vs already on keyframe vs too far)
                try:
                    ...
                    # Helper to pick candidate per mode
                ...
                try:
                    ...
                    for _rec in _data.get("chapters", []):
                        ...
                        if d:
                            _moved.append(int(round(d/1_000_000)))
                    if _moved:
...
def extract_audio_chunk(source_file: str, output_wav: str, start_time: float, duration: float, logger, stream_index: int):
    cmd = ["ffmpeg", "-y", "-v", "error",
           "-ss", str(start_time), "-i", str(source_file),
           "-map", f"0:a:{stream_index}", "-t", str(duration),
...
def find_audio_delay(ref_wav: str, sec_wav: str, logger):
    try:
        ...
        match_pct = (np.max(np.abs(corr)) / (norm + 1e-9)) * 100.0
...
def _tokens_for_track(track: Dict[str, Any], group: str, delays_ms: Dict[str, int], default_flag: Optional[bool] = None):
    ...
    # 1) Common per-track fields
    name = track.get("name", "")
    lang = track.get("lang", "")
    ttype = track.get("type", "")
    src   = str(track.get("src", ""))

    # 2) Infer the source group if missing or invalid (fallback from filename)
    ...
    # 3) Compute delay from group + global positive shift
    delay = 0
    if grp == "sec":
    ...
    # 5) Always emit sync for auditability
    toks += ["--sync", f"0:{delay}"]
...
def build_mkvmerge_tokens(plan_json, output_file, chapters_xml_path, attachments, track_order_str=None):
    ...
    # Determine which subtitle (if any) should be default, honoring toggle
    ...
    # Build grouped inputs with explicit defaults and languages
    ...
    # attachments: no compression options here
    for a in (attachments or []):
        tokens += ["--attach-file", str(a)]

    # Log final per-track delays after applying global shift
...
def write_mkvmerge_json_options(tokens: List[str], json_path: Path, logger) -> str:
    ...
    # Optional logging of the constructed options
...
def run_videodiff(ref: str, target: str, logger, videodiff_path: Path | str) -> Tuple[int, float]:
    ...
    """
    # Resolve path: explicit CONFIG path > PATH > script dir
    ...
    # Find the last [Result] line containing itsoffset/ss
...
def merge_job(ref_file: str, sec_file: Optional[str], ter_file: Optional[str], out_dir: str, logger, videodiff_path: Path):
    ...
    set_status("Analyzing…"); set_progress(0.0)

    delay_sec = None; delay_ter = None
    if sec_file:
        if CONFIG["analysis_mode"] == "VideoDiff":
            ...
            results = run_audio_correlation_workflow(ref_file, sec_file, logger,
                                                     CONFIG["scan_chunk_count"], CONFIG["scan_chunk_duration"],
                                                     lang, role_tag="sec")
    ...
    if ter_file:
        if CONFIG["analysis_mode"] == "VideoDiff":
            ...
            results = run_audio_correlation_workflow(ref_file, ter_file, logger,
                                                     CONFIG["scan_chunk_count"], CONFIG["scan_chunk_duration"],
                                                     lang, role_tag="ter")
            best = best_from_results(results, CONFIG["min_match_pct"])
            if not best:
    ...
    if dpg.does_item_exist("sec_delay_val"): dpg.set_value("sec_delay_val", format_delay_ms(delay_sec))
    if dpg.does_item_exist("ter_delay_val"): dpg.set_value("ter_delay_val", format_delay_ms(delay_ter))

    if CONFIG["workflow"] == "Analyze Only":
        set_status("Analysis complete (no merge)."); set_progress(1.0)
    ...
    # Lossless-only delays: compute global positive shift so no track starts < 0
    present = [0]
    ...
    merge_ok = False
    try:
        set_status("Preparing merge…"); set_progress(0.05)
    ...
    for fr in data.get("frames", []):
        try:
            ...
            if t is None:
                continue
            sec = float(t)
            ns  = int(round(sec * 1_000_000_000))
            ns += int(shift_ms) * 1_000_000  # apply global shift (ms → ns)
            kfs_ns.append(ns)
        except Exception:
            continue
    kfs_ns.sort()
...
def _snap_chapter_times_inplace(root, keyframes_ns: list[int], starts_only: bool,
                                mode: str, threshold_ms: int, logger, sidecar_path: str|None,
                                global_shift_ms: int):
    ...
    def _pick(ts_ns: int) -> tuple[int,int]:
        ...
        if not keyframes_ns:
            return ts_ns, 0
        i = bisect.bisect_right(keyframes_ns, ts_ns)  # first > ts
    ...
    for idx, atom in enumerate(atoms, start=1):
        ...
        if st is not None and st.text:
            ...
            # Ensure end > start if both present
            if en is not None and en.text:
                try:
                    e_now = _parse_ns(en.text)
                    s_now = _parse_ns(st.text)
                    if e_now <= s_now:
                        en.text = _fmt_ns(s_now + 1_000_000)  # +1 ms safety
                except Exception:
                    pass
    ...
    if sidecar_path:
        try:
            with open(sidecar_path, "w", encoding="utf-8") as f:
                json.dump({
                    "global_shift_ms": int(global_shift_ms),
                    "mode": mode,
                    "threshold_ms": int(threshold_ms),
                    "starts_only": bool(starts_only),
                    "chapters": records
                }, f, indent=2)
...
def worker_run_jobs(and_merge=False):
    # Hand off one or more matching jobs based on user’s paths.
    try:
        find_required_tools()
    except Exception as e:
        LOG_Q.put(str(e))
    ...
    set_status("Preparing jobs…"); set_progress(0.0)
    ...
    set_status("All jobs finished."); set_progress(1.0)
...
def discover_jobs(ref_path, sec_path, ter_path):
    ...
    if ref.is_file():
        return [(str(ref), str(sec) if (sec and sec.is_file()) else None,
                          str(ter) if (ter and ter.is_file()) else None)]
...
def build_ui():
    ...
    # ---- Per-item readability (inputs only): larger font + taller padding ----
    
    INPUT_FONT_TAG = None
    try:
        with dpg.font_registry():
            # Try common system fonts on Linux (including Arch/Endeavour layouts)
            _candidates = [
                "/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf",
                "/usr/share/fonts/TTF/DejaVuSans.ttf",
            ...
            for _fp in _candidates:
                try:
                    ...
                    if os.path.exists(_fp):
                        dpg.add_font(_fp, 18, tag="input_font")  # bigger, crisp
                        INPUT_FONT_TAG = "input_font"
                        break
                except Exception:
                    pass

            # Fallback: scan /usr/share/fonts recursively for a TTF/OTF
    ...
    # Theme only for input boxes (taller without affecting whole UI)
    try:
    ...
    # Theme only for controls (combos, buttons, progress): taller via padding
    try:
        with dpg.theme(tag="control_theme"):
            ...
            with dpg.theme_component(dpg.mvProgressBar):
                # Some backends ignore padding here; we will also set explicit height on creation
    ...
    def _safe_bind_input_enhancements(item_tag: str):
        try:
            if dpg.does_item_exist(item_tag):
                # bind theme first (height/padding)
                try:
                    dpg.bind_item_theme(item_tag, "input_theme")
                except Exception:
                    pass
                # bind bigger font just for inputs (if added)
                if INPUT_FONT_TAG:
                    try:
    ...
    with dpg.window(tag="main_window", label=APP_NAME, width=1180, height=780):
        ...
        with dpg.group(horizontal=True):
            ...
            if INPUT_FONT_ID: dpg.bind_item_font("ref_input", INPUT_FONT_ID)
        ...
        with dpg.group(horizontal=True):
            ...
            if INPUT_FONT_ID: dpg.bind_item_font("sec_input", INPUT_FONT_ID)
        ...
        with dpg.group(horizontal=True):
            ...
            if INPUT_FONT_ID: dpg.bind_item_font("ter_input", INPUT_FONT_ID)
        ...
        with dpg.group(horizontal=True):
            ...
            dpg.add_combo(tag="workflow_combo", items=["Analyze & Merge", "Analyze Only"],
                          default_value=CONFIG["workflow"], width=260)
            _bind_control_theme("workflow_combo")
            dpg.add_spacer(width=12)
            dpg.add_text("Mode")
            dpg.add_combo(tag="mode_combo", items=["Audio Correlation", "VideoDiff"],
                          default_value=CONFIG["analysis_mode"], width=260)
        ...
        # File dialogs
        with dpg.file_dialog(tag="file_dialog_ref", label="Pick Reference", callback=on_pick_file, user_data="ref_input",
                             width=700, height=400, directory_selector=False, show=False):
            dpg.add_file_extension(".*", color=(150, 255, 150, 255))
        with dpg.file_dialog(tag="file_dialog_sec", label="Pick Secondary", callback=on_pick_file, user_data="sec_input",
                             width=700, height=400, directory_selector=False, show=False):
            dpg.add_file_extension(".*", color=(150, 255, 150, 255))
        with dpg.file_dialog(tag="file_dialog_ter", label="Pick Tertiary", callback=on_pick_file, user_data="ter_input",
                             width=700, height=400, directory_selector=False, show=False):
            dpg.add_file_extension(".*", color=(150, 255, 150, 255))
        with dpg.file_dialog(tag="dir_dialog_out", label="Pick Output Directory", callback=on_pick_dir, user_data="out_input",
                             width=700, height=400, directory_selector=True, show=False):
            pass
        with dpg.file_dialog(tag="dir_dialog_temp", label="Pick Temp Directory", callback=on_pick_dir, user_data="temp_input",
                             width=700, height=400, directory_selector=True, show=False):
            pass
        with dpg.file_dialog(tag="file_dialog_vdiff", label="Pick videodiff binary", callback=on_pick_file, user_data="vdiff_input",
                             width=700, height=400, directory_selector=False, show=False):
            dpg.add_file_extension(".*", color=(150, 255, 150, 255))
        # Storage modal (Temp + Output + Videodiff) — labels BEFORE fields; taller inputs
        with dpg.window(tag="storage_modal", label="Storage Settings", modal=True, show=False, no_resize=False, width=800, height=300):
            ...
            with dpg.group(horizontal=True):
                ...
                if INPUT_FONT_ID: dpg.bind_item_font("temp_input", INPUT_FONT_ID)
            ...
            with dpg.group(horizontal=True):
                ...
                if INPUT_FONT_ID: dpg.bind_item_font("out_input", INPUT_FONT_ID)
            ...
            with dpg.group(horizontal=True):
                ...
                if INPUT_FONT_ID: dpg.bind_item_font("vdiff_input", INPUT_FONT_ID)
        ...
        # Global Options modal — moved checkbox settings here (labels before boxes)
        with dpg.window(tag="global_modal", label="Global Options", modal=True, show=False, no_resize=False, width=720, height=280):
            ...
            with dpg.group(horizontal=True):
                ...
                dpg.add_combo(tag="snap_mode_opt", items=["previous", "nearest"],
                              default_value=CONFIG.get("snap_mode", "previous"), width=140)
            with dpg.group(horizontal=True):
                dpg.add_text("Max snap distance (ms)")
                dpg.add_input_int(tag="snap_threshold_ms", default_value=int(CONFIG.get("snap_threshold_ms", 250)),
        ...
        # Analysis Settings modal (Audio CC + VideoDiff thresholds) — labels BEFORE inputs
