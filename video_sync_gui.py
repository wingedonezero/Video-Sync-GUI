...
try:
    ...
    def _vsg_apply_appearance_once():
        try:
            ...
            pass
    try:
        dpg.set_frame_callback(10, _vsg_apply_appearance_once)
    except Exception:
        pass
except Exception:
    pass
...
def ffprobe_duration(path: str, logger) -> float:
    out = run_command(['ffprobe', '-v', 'error', '-show_entries', 'format=duration', '-of', 'csv=p=0', str(path)],
                      logger)
    try:
...
def _probe_keyframes_ns(ref_video_path: str, shift_ms: int, logger) -> list[int]:
    ...
    args = ['ffprobe', '-v', 'error', '-select_streams', 'v:0', '-skip_frame', 'nokey', '-show_entries',
            'frame=pkt_pts_time,best_effort_timestamp_time,key_frame', '-of', 'json', ref_video_path]
    ...
    with dpg.window(tag='main_window', label=APP_NAME, width=1180, height=780):
        with dpg.group(tag='header_options_row'):
            ...
            with dpg.file_dialog(tag='file_dialog_vdiff', label='Pick videodiff binary', callback=on_pick_file,
                ...
                dpg.add_file_extension('.*', color=(150, 255, 150, 255))
            with dpg.window(tag='storage_modal', label='Storage Settings', modal=True, show=False, no_resize=False,
                ...
                with dpg.group(horizontal=True):
                    ...
                    dpg.add_input_text(tag='temp_input', label='', default_value=CONFIG['temp_root'], width=600,
                ...
                with dpg.group(horizontal=True):
                    ...
                    dpg.add_input_text(tag='out_input', label='', default_value=CONFIG['output_folder'], width=600,
                                       multiline=False, height=40)
                    if INPUT_FONT_ID:
                ...
                with dpg.group(horizontal=True):
                    ...
                    dpg.add_input_text(tag='vdiff_input', label='', default_value=CONFIG.get('videodiff_path', ''),
                                       width=600, multiline=False, height=40)
                    if INPUT_FONT_ID:
                ...
                dpg.add_separator()
                with dpg.group(horizontal=True):
                    dpg.add_button(label='Save', callback=lambda s, a, u: (ui_save_settings(),
                                                                           dpg.configure_item('storage_modal',
                                                                                              show=False)))
                    dpg.add_button(label='Cancel', callback=lambda: dpg.configure_item('storage_modal', show=False))
            with dpg.window(tag='global_modal', label='Global Options', modal=True, show=False, no_resize=False,
                ...
                with dpg.group(horizontal=True):
                    ...
                    dpg.add_checkbox(tag='firstsubdef_chk', label='',
                                     default_value=bool(CONFIG.get('first_sub_default', True)))
                dpg.add_separator()
                with dpg.group(horizontal=True):
                    dpg.add_button(label='Save', callback=lambda s, a, u: (ui_save_settings(),
                                                                           dpg.configure_item('global_modal',
                                                                                              show=False)))
                ...
                with dpg.group(horizontal=True):
                    ...
                    dpg.add_checkbox(tag='snap_chapters_chk', label='',
                ...
                with dpg.group(horizontal=True):
                    ...
                    dpg.add_checkbox(tag='snap_starts_only_chk', label='',
                                     default_value=bool(CONFIG.get('snap_starts_only', True)))
            with dpg.window(tag='analysis_modal', label='Analysis Settings', modal=True, show=False, no_resize=False,
                ...
                with dpg.group(horizontal=True):
                    dpg.add_text('Chunks')
                    dpg.add_input_int(tag='chunks_input', label='', default_value=int(CONFIG['scan_chunk_count']),
                                      min_value=1, max_value=50, width=100)
                    dpg.add_spacer(width=12)
                    dpg.add_text('Chunk Dur (s)')
                    dpg.add_input_int(tag='chunkdur_input', label='', default_value=int(CONFIG['scan_chunk_duration']),
                                      min_value=2, max_value=120, width=120)
                    dpg.add_spacer(width=12)
                    dpg.add_text('Min Match %')
                    dpg.add_input_float(tag='thresh_input', label='', default_value=float(CONFIG['min_match_pct']),
                                        width=120)
                dpg.add_text('VideoDiff')
                with dpg.group(horizontal=True):
                    dpg.add_text('Min error')
                    dpg.add_input_float(tag='vd_err_min', label='',
                                        default_value=float(CONFIG.get('videodiff_error_min', 0.0)), width=100,
                                        format='%.2f')
                    dpg.add_spacer(width=12)
                    dpg.add_text('Max error')
                    dpg.add_input_float(tag='vd_err_max', label='',
                                        default_value=float(CONFIG.get('videodiff_error_max', 100.0)), width=100,
                                        format='%.2f')
                dpg.add_separator()
                with dpg.group(horizontal=True):
                    dpg.add_button(label='Save', callback=lambda s, a, u: (ui_save_settings(),
                                                                           dpg.configure_item('analysis_modal',
                                                                                              show=False)))
...
def _vsg_parse_mkvmerge_tokens(tokens: list[str]) -> dict:
    ...
    def add_input(pth: str, opts: list[str]):
        ...
        it = iter(opts)
        for t in it:
            if t in ("--language", "--track-name", "--default-track-flag", "--forced-track-flag", "--sync",
                     "--compression"):
                try:
                    d[t] = next(it)
