...
pub trait I18nAssets {
    ...
    fn filenames_iter(&self) -> Box<dyn Iterator<Item = String> + 'static>;
    ...
    T: RustEmbed,
{
    ...
    fn filenames_iter(&self) -> Box<dyn Iterator<Item = String> + 'static> {
        let filenames: Vec<String> = Self::iter().map(|filename| filename.to_string()).collect();
    ...
    T: RustEmbed,
{
    ...
    fn filenames_iter(&self) -> Box<dyn Iterator<Item = String> + 'static> {
        let filenames: Vec<String> = T::iter().map(|filename| filename.to_string()).collect();
        Box::new(filenames.into_iter())
    }
...
impl I18nAssets for FileSystemAssets {
    ...
    fn filenames_iter(&self) -> Box<dyn Iterator<Item = String> + 'static> {
        ...
        let filenames: Vec<String> = walkdir::WalkDir::new(&base_dir)
            .into_iter()
...
impl I18nAssets for AssetsMultiplexor {
    ...
    fn filenames_iter(&self) -> Box<dyn Iterator<Item = String> + 'static> {
        let filenames: Vec<String> = self.assets
            .iter()
