...
def test_float_precision_through_pipeline():
    ...
    assert (
        abs(event.start_ms - expected) < 0.001
...
def test_style_operations():
    ...
    assert (
        data.styles["Default"].fontsize == 72.0
    ), f"Expected 72.0, got {data.styles['Default'].fontsize}"
    assert (
        data.styles["Signs"].fontsize == 54.0
...
def test_mkvmerge_sync_mode():
    ...
    for i, event in enumerate(data.events):
        assert (
            event.start_ms == original_starts[i]
