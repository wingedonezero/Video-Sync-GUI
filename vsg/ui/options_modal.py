...
def _row_combo(label, tag, key, items, tip=""):
    with dpg.group(horizontal=True):
        ...
        dpg.add_combo(tag=tag, items=items, width=260,
                      ...
                      callback=lambda s, a: (B.on_changed(s, a),
                                             _apply_mode_visibility() if key == 'analysis_mode' else None))
        _tip(tip);
        B.bind(tag, key)
...
def _apply_mode_visibility():
    ...
    if dpg.does_item_exist(PFX + "op_mode"):
        try:
            mode = dpg.get_value(PFX + "op_mode")
        except Exception:
            pass
    m = str(mode).lower()
            ...
            with dpg.tab(label="Save / Load"):
                ...
                with dpg.group(horizontal=True):
                    dpg.add_button(label="Save",
                                   callback=lambda *_: (_ui_log(f"Saved settings -> {SETTINGS_PATH}"), save_settings()))
                    dpg.add_button(label="Load", callback=lambda *_: (_ui_log(f"Loading settings <- {SETTINGS_PATH}"),
                                                                      load_settings(), adopt_into_app(), B.refresh(),
                                                                      _apply_mode_visibility()))
