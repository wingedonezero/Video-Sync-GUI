...
class JobQueueLogic:
    ...
    def add_jobs(self, new_jobs: list[dict]) -> None:
        ...
        new_jobs.sort(
            key=lambda j: natural_sort_key(Path(j["sources"]["Source 1"]).name)
        )
    ...
    def _update_row(self, row: int, job: dict) -> None:
        ...
        status_text = (
            "Configured"
            if self.layout_manager.layout_exists(job_id)
        ...
        if status_text == "Configured":
            ...
            if layout_data:
                ...
                if sync_exclusion_issues:
                    all_issues.extend(
                        [f"Sync Exclusion: {issue}" for issue in sync_exclusion_issues]
                    )
                if style_edit_issues:
                    all_issues.extend(
                        [f"Style Edit: {issue}" for issue in style_edit_issues]
                    )
        ...
        for track in enhanced_layout:
            ...
            try:
                ...
                temp_dir = (
                    ...
                    / f"vsg_gen_validate_{job_id}_{source_key}_{source_id}_{timestamp}"
                )
                ...
                try:
                    extracted = extract_tracks(
                        ...
                        self.tool_paths,
                        "validate",
                        specific_tracks=[source_id],
                    )
                    if not extracted:
                        issues.append(
                            f"'{track_name}': Could not extract source track for validation"
                        )
                        continue

                    # Get available styles from the extracted file
                    available_styles = SubtitleData.get_style_counts_from_file(
                        extracted[0]["path"]
                    )
                    ...
                    if filter_styles:
                        missing_filter_styles = [
                            s for s in filter_styles if s not in available_style_set
                        ]
                    ...
                    if original_style_set != available_style_set:
                        ...
                        if missing_styles and not extra_styles:
                            ...
                            track["original_style_list"] = sorted(
                        ...
                        # This is RISKY because extra styles might contain unwanted dialogue
                            ...
                            if missing_styles:
                                warning_parts.append(
                                    f"Missing: {', '.join(sorted(missing_styles))}"
                                )
                            warning_parts.append(
                                f"Extra: {', '.join(sorted(extra_styles))}"
                            )
                            issues.append(
                                f"'{track_name}': Style set mismatch ({'; '.join(warning_parts)})"
                            )
        ...
        for track in enhanced_layout:
            ...
            if track.get("perform_ocr"):
                ...
                if font_replacements:
                    ...
                    missing_replacement_styles = (
                        replacement_styles - available_style_set
                    )
            ...
            try:
                ...
                temp_dir = (
                    Path(tempfile.gettempdir())
                    / f"vsg_style_validate_{job_id}_{source_key}_{track_id}_{timestamp}"
                )
                temp_dir.mkdir(parents=True, exist_ok=True)
                ...
                try:
                    ...
                    if not extracted:
                        issues.append(
                            f"'{track_name}': Could not extract track for style validation"
                        )
                        continue
        ...
        for track in enhanced_layout:
            ...
            try:
                ...
                if track.get("is_generated"):
                    ...
                    temp_dir = (
                        Path(tempfile.gettempdir())
                        / f"vsg_sync_validate_{job_id}_{source_key}_{source_id}_{timestamp}"
                    )
                    temp_dir.mkdir(parents=True, exist_ok=True)
                    ...
                    if not subtitle_path:
                        issues.append(
                            f"'{track_name}': Could not extract track for sync exclusion validation"
                        )
                        import shutil
                    ...
                    temp_dir = (
                        Path(tempfile.gettempdir())
                        / f"vsg_sync_validate_{job_id}_{source_key}_{track_id}_{timestamp}"
                    )
                    temp_dir.mkdir(parents=True, exist_ok=True)
                ...
                try:
                    ...
                    if exclusion_styles:
                        missing_exclusion_styles = [
                            s for s in exclusion_styles if s not in available_style_set
                        ]
                    ...
                    if original_style_set != available_style_set:
                        ...
                        if missing_styles and not extra_styles:
                            track["sync_exclusion_original_style_list"] = sorted(
                                available_styles.keys()
                            )
                            layout_modified = True
                        ...
                        # WARN: If ANY extra styles exist, require manual review
                            ...
                            issues.append(
                                f"'{track_name}': Sync exclusion style mismatch ({'; '.join(warning_parts)})"
                            )
                ...
                # If validation fails, warn but don't block
                issues.append(
                    f"'{track_name}': Could not validate sync exclusion styles ({e!s})"
                )
        ...
        if "track_info" not in job or job["track_info"] is None:
            try:
                job["track_info"] = get_track_info_for_dialog(
                    job["sources"], self.runner, self.tool_paths
                )
            except Exception as e:
                QMessageBox.critical(
                    self.v,
                    "Error Analyzing Tracks",
                    f"Could not analyze tracks for {Path(job['sources']['Source 1']).name}:\n{e}",
                )
                return None
    ...
    def configure_job_at_row(self, row: int) -> None:
        ...
        previous_layout = (
            self._convert_enhanced_to_dialog_format(
                existing_layout.get("enhanced_layout")
            )
            if existing_layout
            else []
        )
        previous_attachments = (
            existing_layout.get("attachment_sources", []) if existing_layout else []
        )
            ...
            if layout:
                ...
                if save_ok:
                    ...
                    QMessageBox.critical(
                        self.v,
                        "Save Failed",
                        "Could not save the job layout. Check log for details.",
                    )
    ...
    def copy_layout(self, source_job_index: int) -> None:
        ...
        if layout_data:
            ...
            self.v.log_callback(
                f"[Queue] Copied layout from {Path(job['sources']['Source 1']).name}."
            )
        else:
            QMessageBox.warning(
                self.v, "Copy Failed", "Could not load the layout file to copy."
            )
    ...
    def paste_layout(self) -> None:
        ...
        if not self._layout_clipboard:
            QMessageBox.warning(
                self.v,
                "Paste Failed",
                "Clipboard is empty. Please copy a layout first.",
            )
            return
        ...
        if not selected_indices:
            QMessageBox.information(
                self.v,
                "No Selection",
                "Please select one or more target jobs to paste to.",
            )
            return
        ...
        for target_index in selected_indices:
            ...
            if not target_track_info:
                continue

            target_struct_sig = (
                self.layout_manager.signature_gen.generate_structure_signature(
                    target_track_info
                )
            )

            if self.layout_manager.signature_gen.structures_are_compatible(
                source_struct_sig, target_struct_sig
            ):
                new_layout = self._replace_paths_in_layout(
                    self._layout_clipboard["enhanced_layout"], target_job["sources"]
                )

                target_job_id = self.layout_manager.generate_job_id(
                    target_job["sources"]
                )
                save_ok = self.layout_manager.save_job_layout(
                    target_job_id,
                    new_layout,
                    self._layout_clipboard["attachment_sources"],
                    target_job["sources"],
                    target_track_info,
                ...
                self.v.log_callback(
                    f"Skipped pasting to {Path(target_job['sources']['Source 1']).name}: Incompatible track structure."
                )
        ...
        if updated_count > 0:
            QMessageBox.information(
                self.v,
                "Paste Successful",
                f"Successfully pasted layout to {updated_count} job(s).",
            )
        else:
            QMessageBox.warning(
                self.v,
                "Paste Failed",
                "Could not paste layout to any selected jobs due to incompatible track structures.",
            )
    ...
    def _replace_paths_in_layout(
        ...
        for track_template in layout_template:
            ...
            filter_config = new_track.get("filter_config")
            if filter_config:
                filter_config.pop("kept_indices", None)
                filter_config.pop("forced_include", None)
                filter_config.pop("forced_exclude", None)
    ...
    def get_final_jobs(self) -> list[dict]:
        ...
        for job in self.jobs:
            ...
            layout_data = self.layout_manager.load_job_layout(job_id)
            if layout_data:
                job["manual_layout"] = self._convert_enhanced_to_dialog_format(
                    layout_data["enhanced_layout"]
                )
        ...
        if unconfigured_names:
            QMessageBox.warning(
                self.v,
                "Unconfigured Jobs",
                f"{len(unconfigured_names)} job(s) are not configured and will be skipped.",
            )
