...
class SourceList(QListWidget):
    ...
    def add_track_item(self, track: dict, guard_block: bool):
        item_text = (
            f"[{track['type'][0].upper()}-{track['id']}] {track.get('description', '')}"
        )
    ...
    def _show_context_menu(self, pos: QPoint):
        ...
        is_text_subtitle = track.get("type") == "subtitles" and track.get(
            "codec_id", ""
        ).upper() in ["S_TEXT/UTF8", "S_TEXT/ASS", "S_TEXT/SSA"]
...
class FinalList(QListWidget):
    ...
    def add_track_widget(self, track_data: dict, preset=False):
        ...
        if hasattr(widget, "style_editor_btn"):
            widget.style_editor_btn.clicked.connect(
                lambda checked, w=widget: self.dialog._launch_style_editor(w)
            )
        ...
        if preset:
            ...
            if "S_TEXT/UTF8" in (getattr(widget, "codec_id", "") or "").upper():
                if hasattr(widget, "cb_convert"):
                    widget.cb_convert.setChecked(
                        track_data.get("convert_to_ass", False)
                    )
        ...
        if hasattr(widget, "cb_default"):
            widget.cb_default.clicked.connect(
                lambda checked, w=widget: self._enforce_single_default(w)
            )
    ...
    def _show_context_menu(self, pos: QPoint):
        ...
        is_generated = (
            widget.track_data.get("is_generated", False)
            if hasattr(widget, "track_data")
            else False
        )
        ...
        act_copy.setEnabled(
            is_subs
            and bool(
                widget.track_data.get("style_patch")
                or widget.track_data.get("font_replacements")
            )
        )
        ...
        if act == act_up:
            ...
            self.dialog._copy_style_edits(widget)
        elif act == act_paste:
            self.dialog._paste_style_edits(widget)
