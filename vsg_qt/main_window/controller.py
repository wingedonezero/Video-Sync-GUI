...
class MainController:
    ...
    def apply_config_to_ui(self):
        v = self.v
        # These inputs are now for the "Quick Analysis" section only
    ...
    def browse_for_path(self, line_edit, caption: str):
        ...
        # Use a more generic starting directory if possible
    ...
    def _run_configured_jobs(self, final_jobs: List[Dict]):
        ...
        # A batch is defined as multiple jobs being run together.
        is_batch = len(final_jobs) > 1

        if is_batch:
             # If the reference source is a directory, use its name.
             # If it's a file, use its parent directory's name.
    ...
    def start_batch_analyze_only(self):
        ...
        is_batch = Path(source1_path_str).is_dir() if source1_path_str else len(initial_jobs) > 1
        if is_batch:
    ...
    def job_finished(self, result: dict):
        ...
        # DYNAMIC FIX: Iterate through all available delay labels and update them
        # This assumes the labels were created in the order "Source 2", "Source 3", etc.
        for i, label in enumerate(self.v.delay_labels):
            source_key = f"Source {i + 2}"
            delay_val = delays.get(source_key)
            if delay_val is not None:
                label.setText(f"{delay_val} ms")
            else:
                label.setText("—") # Clear if not present in this job's results
    ...
    def batch_finished(self, all_results: list):
        ...
        # Determine if it was a batch run and get the output directory
        is_batch = len(all_results) > 1
        ref_path_for_batch_check = ""
        if all_results and all_results[0].get('job_data_for_batch_check'):
            ref_path_for_batch_check = all_results[0]['job_data_for_batch_check'].get('ref_path_for_batch_check', '')

        if not is_batch and ref_path_for_batch_check and Path(ref_path_for_batch_check).is_dir():
            is_batch = True

        if all_results:
            for res in all_results:
                if res.get('status') in ['Merged', 'Analyzed'] and 'output' in res and res['output']:
                    output_dir = Path(res['output']).parent
                    break
