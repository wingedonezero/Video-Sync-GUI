...
class SubtitleSyncTab(QWidget):
    def __init__(self):
        ...
        self.widgets["video_verified_frame_remap"] = QCheckBox()
        self.widgets["video_verified_frame_remap"].setChecked(False)
        self.widgets["video_verified_frame_remap"].setToolTip(
            "Preserve frame position during sync:\n\n"
            "OFF (Default): Apply delay as milliseconds\n"
            "  - Simple offset addition to all timestamps\n"
            "  - May have minor rounding at frame boundaries\n\n"
            "ON: Remap using frame + centisecond position\n"
            "  - Preserves exact position within each frame\n"
            "  - Guarantees duration in frames is preserved\n"
            "  - No rounding issues at frame boundaries"
        )
        ...
        specific_layout.addRow(
            "Preserve Frame Position:", self.widgets["video_verified_frame_remap"]
        )
        ...
        self.widgets["interlaced_handling_enabled"].stateChanged.connect(
            self._update_interlaced_visibility
        )

        self.widgets["interlaced_force_mode"] = QComboBox()
        self.widgets["interlaced_force_mode"].addItems(
            ["auto", "interlaced", "telecine", "progressive"]
        )
        ...
        self.widgets["interlaced_hash_algorithm"].addItems(
        ...
        self.widgets["interlaced_hash_size"].setToolTip(
            "Hash size for interlaced content (8 recommended)"
        )
        ...
        self.widgets["interlaced_num_checkpoints"].setToolTip(
            "Number of checkpoint times for interlaced content"
        )
        ...
        self.widgets["interlaced_deinterlace_method"].addItems(
            ["bwdif", "yadif", "yadifmod", "bob", "w3fdif"]
        )
        ...
        interlaced_layout.addRow(
            "Enable Interlaced Handling:", self.widgets["interlaced_handling_enabled"]
        )
        interlaced_layout.addRow(
            "Detection Mode:", self.widgets["interlaced_force_mode"]
        )
        interlaced_layout.addRow(
            "Hash Algorithm:", self.widgets["interlaced_hash_algorithm"]
        )
        interlaced_layout.addRow("Hash Size:", self.widgets["interlaced_hash_size"])
        interlaced_layout.addRow(
            "Hash Threshold:", self.widgets["interlaced_hash_threshold"]
        )
        interlaced_layout.addRow(
            "Sequence Length:", self.widgets["interlaced_sequence_length"]
        )
        interlaced_layout.addRow(
            "Checkpoints:", self.widgets["interlaced_num_checkpoints"]
        )
        interlaced_layout.addRow(
            "Search Range:", self.widgets["interlaced_search_range_frames"]
        )
        interlaced_layout.addRow(
            "Deinterlace Method:", self.widgets["interlaced_deinterlace_method"]
        )
        interlaced_layout.addRow(
    ...
    def _update_mode_visibility(self, text: str):
        ...
        self.widgets["video_verified_frame_remap"].setEnabled(is_video_verified)
    ...
    def _update_interlaced_visibility(self):
        ...
        is_video_verified = (
            self.widgets["subtitle_sync_mode"].currentText() == "video-verified"
        )
        interlaced_enabled = (
            self.widgets["interlaced_handling_enabled"].isChecked()
            and is_video_verified
        )
